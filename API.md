...
| type | Meaning |
|---|---|
| `hello` | Client intro: `pushToken`, `connectors` (calendar/reminders/…), optional phone tool schemas |
| `audio` | 16kHz Int16 PCM chunk (base64) — backend runs local Whisper |
| `transcript` | Typed text turn (`typed: true` skips the wake-word gate) |
| `mute` | `{on: bool}` — backend drops audio frames pre-Whisper |
...
| type | Meaning |
|---|---|
| `ready` | Hello fully processed — mic can arm |
...
| `tool_call` | Backend asking the phone to run an on-device tool (calendar, reminders…) |
...
Notifications: `POST /api/notify` (send one), `GET /api/notifications/recent`
...
(`{time, title, body, kind}` — `HH:MM` or ISO), `POST /api/proactive/cancel`,
...
`POST /api/internal/mcp_relay/{token}` (loopback only — phone-tool relay for CLI
brains).
